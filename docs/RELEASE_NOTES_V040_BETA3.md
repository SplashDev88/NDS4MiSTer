...
- **Faster 3D graphics transfers.** The new write-combining (WC) memory path cuts the time spent sending rendered graphics from the ARM processor to the FPGA. The speed improvement varies by game.
- **Enabled automatically.** Run Kickstart as usual. The included WC driver loads on compatible MiSTer kernels. If it cannot load, the helper falls back to the previous memory path.
- **Earlier fixes carry over.** Stability and ROM-switching improvements, smoother movies, both TATE directions, optional Engine B, touchscreen, sound, and cartridge saves are retained.
- **No overclock.** The ARM processor still runs at 1 GHz.
...
5. Within five minutes, go to **Console → NDS_20260916** and choose **Load NDS**. If you miss the window, run Kickstart again.
...
- **Strange Journey can still freeze during its intro.** This performance update does not fix that issue.
- **The WC driver has been tested on MiSTer Linux 5.15.1.** Other kernel builds may use the fallback path and miss the speed improvement. This release does not replace your Linux kernel.
...
This core builds on CPU, graphics, sound, and compatibility work from the MiSTer community, FPGAzumSpass, the Nitro_DarkSide and melonDS contributors, heni, and InsaneFriend. Thanks also to skmp, the DreamSTer developer, for the write-combining suggestion. Full credits and licenses are in the source archive.
