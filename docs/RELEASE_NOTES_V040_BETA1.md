...
**TATE mode now rotates both directions.**
...
The MiSTer menu doesn't follow the core's rotation — you set that separately in `MiSTer.ini` on your SD card. Add this at the end of the file, using `osd_rotate=1` to go with **90 CW** or `osd_rotate=2` to go with **90 CCW**:
...
If an `[NDS]` section is already there, edit that one instead of adding a second. Save the file and reboot. Putting the setting under `[NDS]` keeps it to this core, so the menu stays normal everywhere else. The installer won't touch your INI file — this step is yours to make. ([MiSTer INI documentation](https://mister-devel.github.io/MkDocs_MiSTer/advanced/ini/))
