...
#endif
    Num(num), // well uh
    LockstepTraceFile(nullptr),
    LockstepTraceSequence(0),
    LockstepTraceLimit(0),
    LockstepTraceBusOnly(false),
    LockstepTraceHpsOnly(false),
    LockstepTraceLowPcOnly(false),
    LockstepTraceIrqFocus(false),
    LockstepTracePcRange(false),
    LockstepTracePcMin(0),
    LockstepTracePcMax(0),
    LockstepTraceTriggerEnabled(false),
    LockstepTraceTriggered(false),
    LockstepTraceTriggerPc(0),
    NDS(nds)
{
    if (const char* base = getenv("NDS4MISTER_ARM_TRACE"))
    {
        char path[1024];
        snprintf(path, sizeof(path), "%s.%s.jsonl", base, Num ? "arm7" : "arm9");
        LockstepTraceFile = fopen(path, "w");
        const char* limit = getenv("NDS4MISTER_ARM_TRACE_LIMIT");
        LockstepTraceLimit = limit ? strtoull(limit, nullptr, 0) : 1000000;
        LockstepTraceBusOnly =
            getenv("NDS4MISTER_ARM_TRACE_BUS_ONLY") != nullptr;
        LockstepTraceHpsOnly =
            getenv("NDS4MISTER_ARM_TRACE_HPS_ONLY") != nullptr;
        LockstepTraceLowPcOnly =
            getenv("NDS4MISTER_ARM_TRACE_LOW_PC_ONLY") != nullptr;
        LockstepTraceIrqFocus =
            getenv("NDS4MISTER_ARM_TRACE_IRQ_FOCUS") != nullptr;
        const char* pcMin = getenv("NDS4MISTER_ARM_TRACE_PC_MIN");
        const char* pcMax = getenv("NDS4MISTER_ARM_TRACE_PC_MAX");
        if (pcMin && pcMax)
        {
            LockstepTracePcMin = strtoul(pcMin, nullptr, 0);
            LockstepTracePcMax = strtoul(pcMax, nullptr, 0);
            LockstepTracePcRange =
                LockstepTracePcMin < LockstepTracePcMax;
        }
        if (const char* trigger = getenv("NDS4MISTER_ARM_TRACE_TRIGGER_PC"))
        {
            LockstepTraceTriggerPc = strtoul(trigger, nullptr, 0);
            LockstepTraceTriggerEnabled = true;
        }
    }
...
ARM::~ARM()
{
    if (LockstepTraceFile)
        fclose(LockstepTraceFile);
}

void ARM::TraceInstructionEntry()
{
    if (!LockstepTraceFile || LockstepTraceBusOnly ||
        LockstepTraceSequence >= LockstepTraceLimit)
        return;
    const u32 pc = R[15] - ((CPSR & 0x20) ? 4u : 8u);
    if (LockstepTraceTriggerEnabled && !LockstepTraceTriggered)
    {
        if (Num != 0 || pc != LockstepTraceTriggerPc)
            return;
        LockstepTraceTriggered = true;
    }
    if (LockstepTraceLowPcOnly &&
        (Num != 0 || pc >= 0x02000000u))
        return;
    if (LockstepTracePcRange)
    {
        if (Num != 0 || pc < LockstepTracePcMin || pc >= LockstepTracePcMax)
            return;
    }
    if (LockstepTraceIrqFocus)
    {
        const bool focused =
            (pc >= 0x02008000u && pc < 0x0201A000u) ||
            (pc >= 0x0200D000u && pc < 0x0200F200u) ||
            (pc >= 0x02024000u && pc < 0x02026000u) ||
            (pc >= 0x02068000u && pc < 0x0206E000u) ||
            (pc >= 0x0207F000u && pc < 0x02081000u) ||
            (pc >= 0x0207CC80u && pc < 0x0207CD20u) ||
            (pc >= 0x01FFA000u && pc < 0x01FFB000u) ||
            (pc >= 0xFFFF0600u && pc < 0xFFFF0800u);
        if (Num != 0 || !focused)
            return;
    }

    fprintf(LockstepTraceFile,
            "{\"event\":\"instruction\",\"cpu\":\"%s\",\"seq\":%llu,"
            "\"pc\":\"%08X\",\"opcode\":\"%08X\",\"thumb\":%s,"
            "\"cpsr\":\"%08X\",\"regs\":[",
            Num ? "arm7" : "arm9",
            static_cast<unsigned long long>(LockstepTraceSequence),
            R[15], CurInstr, (CPSR & 0x20) ? "true" : "false", CPSR);
    for (int i = 0; i < 16; i++)
        fprintf(LockstepTraceFile, "%s\"%08X\"", i ? "," : "", R[i]);
    fputs("]}\n", LockstepTraceFile);
    LockstepTraceSequence++;
}

void ARM::TraceBus(const char* kind, u32 addr, u32 size, u32 value)
{
    if (!LockstepTraceFile || LockstepTraceSequence >= LockstepTraceLimit)
        return;
    if (LockstepTraceTriggerEnabled && !LockstepTraceTriggered)
        return;
    if (LockstepTracePcRange)
    ...
    const u32 tracePC = R[15] - ((CPSR & 0x20) ? 4u : 8u);
    fprintf(LockstepTraceFile,
