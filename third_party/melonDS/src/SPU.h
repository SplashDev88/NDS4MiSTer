...
#ifndef NDS4MISTER_FPGA_AUDIO_OFFLOAD
#define NDS4MISTER_FPGA_AUDIO_OFFLOAD 0
#endif
...
public:
    ...
    // Advance only software-visible channel timing/status. This deliberately
    // performs no sample-memory reads, decoding, interpolation, or mixing.
    bool AdvanceFPGAAudioOffload(u32 cycles);
#endif
...
#if NDS4MISTER_FPGA_AUDIO_OFFLOAD
    // Advance capture timer/status without producing or storing audio data.
    // Capture RAM contents are owned by the FPGA implementation in this mode.
    bool AdvanceFPGAAudioOffload(u32 cycles);
#endif
...
#if NDS4MISTER_FPGA_AUDIO_OFFLOAD
struct FPGAAudioOffloadStats
{
    u64 MixCallbacks = 0;
    u64 HPSRenderCallbacks = 0;
    u64 ChannelAdvanceCallbacks = 0;
    u64 CaptureAdvanceCallbacks = 0;
};
#endif
...
#if NDS4MISTER_FPGA_AUDIO_OFFLOAD
    void SetFPGAAudioOffload(bool enable);
    bool GetFPGAAudioOffload() const { return FPGAAudioOffload; }
    bool SetFPGAAudioTimeAuthority(bool enable);
    bool GetFPGAAudioTimeAuthority() const
    {
        return FPGAAudioTimeAuthority;
    }
    FPGAAudioOffloadStats GetFPGAAudioOffloadStats() const
    {
        return FPGAAudioOffloadCounters;
    }
#endif
...
private:
    ...
    bool FPGAAudioTimeAuthority = false;
    FPGAAudioOffloadStats FPGAAudioOffloadCounters {};
#endif
};
