...
namespace
{
FILE* NDS4MiSTerGXDMAStream = nullptr;
u64 NDS4MiSTerGXDMAStreamSequence = 0;
u64 NDS4MiSTerGXDMAStreamLimit = 0;
bool NDS4MiSTerGXDMAStreamInitialized = false;

void TraceGXFIFODMA(u32 channel, u32 source, u32 destination,
                    u32 value, u32 remaining, u64 timestamp)
{
    if (!NDS4MiSTerGXDMAStreamInitialized)
    {
        NDS4MiSTerGXDMAStreamInitialized = true;
        if (const char* path = std::getenv("NDS4MISTER_GX_DMA_STREAM"))
        {
            NDS4MiSTerGXDMAStream = std::fopen(path, "w");
            const char* limit =
                std::getenv("NDS4MISTER_GX_DMA_STREAM_LIMIT");
            NDS4MiSTerGXDMAStreamLimit =
                limit ? std::strtoull(limit, nullptr, 0) : 500000;
        }
    }
    if (NDS4MiSTerGXDMAStream &&
        NDS4MiSTerGXDMAStreamSequence < NDS4MiSTerGXDMAStreamLimit)
    {
        std::fprintf(
            NDS4MiSTerGXDMAStream,
            "{\"seq\":%llu,\"channel\":%u,\"src\":\"%08x\","
            "\"dst\":\"%08x\",\"value\":\"%08x\","
            "\"remaining\":%u,\"timestamp\":%llu}\n",
            static_cast<unsigned long long>(NDS4MiSTerGXDMAStreamSequence),
            channel, source, destination, value, remaining,
            static_cast<unsigned long long>(timestamp));
        if ((NDS4MiSTerGXDMAStreamSequence & 0x3ffu) == 0)
            std::fflush(NDS4MiSTerGXDMAStream);
    }
    NDS4MiSTerGXDMAStreamSequence++;
}
}
...
void DMA::Run9()
{
    ...
    else
    {
        while (IterCount > 0 && !Stall)
        {
            ...
            if (IsGXFIFODMA)
                TraceGXFIFODMA(Num, CurSrcAddr, CurDstAddr, value,
                               RemCount, NDS.ARM9Timestamp);
