...
public:
    ...
    */
    ...
    // Renderer-only local-LCD seam. This mirrors the FPGA descriptor and
    ...
    u64 ExternalRenderOAMRevision[4] {};

    void MarkExternalRenderVRAM(u32 bank) noexcept
    {
        if (bank < 9)
            ExternalRenderVRAMRevision[bank] =
                ++ExternalRenderMemorySequence;
    }
    void MarkExternalRenderVRAMMask(u32 mask) noexcept
    {
        mask &= 0x1ffu;
        if (!mask) return;
        ...
        for (u32 bank=0; bank<9; ++bank)
            if (mask & (1u << bank))
                ExternalRenderVRAMRevision[bank] = revision;
    }
    void MarkExternalRenderPalette(u32 offset, u32 size) noexcept
    {
        if (!size) return;
        const u64 revision = ++ExternalRenderMemorySequence;
        const u32 first = offset / VRAMDirtyGranularity;
        const u32 last = (offset + size - 1) / VRAMDirtyGranularity;
        for (u32 page=first; page<=last && page<4; ++page)
            ExternalRenderPaletteRevision[page] = revision;
    }
    void MarkExternalRenderOAM(u32 offset, u32 size) noexcept
    {
        ...
        const u32 last = (offset + size - 1) / VRAMDirtyGranularity;
        for (u32 page=first; page<=last && page<4; ++page)
            ExternalRenderOAMRevision[page] = revision;
    }
    ...
    NonStupidBitField<256*1024/VRAMDirtyGranularity> NDS4MiSTerTraceDirtyFlatAOBJ {};
    NonStupidBitField<128*1024/VRAMDirtyGranularity> NDS4MiSTerTraceDirtyFlatBOBJ {};
...
struct RendererSettings
{
    ...
    // Keep the software renderer's native 6-bit channel words instead of
    // expanding them to BGRA8888.  This is used only by the MiSTer full-frame
    // path, which repacks those channels directly to scanout BGR666.
    bool PackedOutput = false;

    // Draw the two independent DS 2D engines concurrently. MiSTer's ARM
    // full-frame path uses this to keep the per-HBlank renderer below the
    // hardware scanline cadence; ordinary melonDS frontends leave it off.
    bool Parallel2D = false;

    // Reuse a software-2D line only when the complete renderer input state
    // and the relevant memory revisions are unchanged.
...
{
    u64 Scanlines = 0;
    u64 ScanlineTotalNs = 0;
    u64 Output3DNs = 0;
    u64 CacheDecisionNs = 0;
    u64 EngineANs = 0;
    u64 EngineBNs = 0;
    u64 CompositeANs = 0;
    u64 CompositeBNs = 0;
    ...
    u64 SpritesANs = 0;
    u64 SpritesBNs = 0;
    u64 ParallelWaitNs = 0;
    u64 ParallelWaitMaxNs = 0;
    u64 ParallelTasks = 0;
    u64 ParallelWorkerSleeps = 0;
    u64 ParallelSpinCompletions = 0;
    ...
    u64 ThreeDFrames = 0;
    u64 ThreeDIdenticalFrames = 0;
    u64 ThreeDCoherenceNs = 0;
    u64 ThreeDClearNs = 0;
    u64 ThreeDSetupNs = 0;
    u64 ThreeDRasterNs = 0;
    u64 ThreeDFinalPassNs = 0;
    u64 ThreeDParallelFrames = 0;
...
public:
    ...
    virtual void SetExternalLineCacheReuse(u32 line, bool engineA,
                                           bool engineB)
        { (void)line; (void)engineA; (void)engineB; }
    virtual bool GetExternalLineCacheResult(bool& engineA, bool& engineB)
        { engineA = false; engineB = false; return false; }
    virtual ExternalRendererStageProfile GetExternalRendererStageProfile() const
        { return {}; }
