...
void GPU::Reset() noexcept
{
    ...
    memset(ExternalRenderPaletteRevision, 0,
           sizeof(ExternalRenderPaletteRevision));
    memset(ExternalRenderOAMRevision, 0,
           sizeof(ExternalRenderOAMRevision));
...
void GPU::StartHBlank(u32 line) noexcept
{
    ...
    if (VCount < 192)
        NDS4MiSTer::EmitPreDrawScroll(NDS.NumFrames, static_cast<u16>(line),
            GPU2D_A.BGXPos[1], GPU2D_A.BGXPos[2]);

    if (VCount < 192)
    {
        NDS4MiSTer::EmitTrace2DScanline(*this, line);

        // draw
        // note: this should start 48 cycles after the scanline start
#if !NDS4MISTER_NO_VIDEO_RENDER && !NDS4MISTER_NO_2D_RENDER
        if (line < 192)
    ...
    bool renderer2DOnly) noexcept
{
    ...
    // These are passive mirrors of the FPGA's post-phase descriptor. The
    // renderer seam below never derives flags, raises IRQ, starts DMA, or
    // schedules an LCD/display-FIFO callback.
    VCount = static_cast<u16>(vcount);
    DispStat[0] = static_cast<u16>(dispstat9);
    DispStat[1] = static_cast<u16>(dispstat7);
    VCountOverride = false;
    NextVCount = VCount;
    NDS.NumFrames = frameSequence;
    // `render` decimates derived pixel work only. The register latches,
    // windows, and GPU3D VBlank below are architectural state and must still
    // advance on an omitted output frame.
    const bool frameStart = kind == 2 || (kind == 0 && line == 0);
    if (resync)
    {
#if !NDS4MISTER_NO_VIDEO_RENDER && !NDS4MISTER_NO_3D_RENDER
        if (!renderer2DOnly)
    ...
    {
        ScreensEnabled = !!(NDS.PowerControl9 & (1<<0));
        // FPGA owns LCD and display-FIFO cadence in this mode.
        RunFIFO = false;
        TotalScanlines = 0;
    }
    ...
    if (kind == 1)
    {
        const bool resetregs = VCount == 262;
        GPU2D_A.UpdateRegistersPreDraw(resetregs);
        GPU2D_B.UpdateRegistersPreDraw(resetregs);
...
void GPU::StartScanline(u32 line) noexcept
{
    ...
    else if (VCount == 192)
    {
        ...
        NDS4MiSTer::EmitVBlank(NDS.NumFrames);
...
bool GPU::MakeVRAMFlat_AOBJCoherent(NonStupidBitField<256*1024/VRAMDirtyGranularity>& dirty) noexcept
{
    if (NDS4MiSTer::Trace2DEnabled())
        for (u32 i=0;i<dirty.DataLength;i++) NDS4MiSTerTraceDirtyFlatAOBJ.Data[i]|=dirty.Data[i];
