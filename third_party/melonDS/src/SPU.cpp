...
#if NDS4MISTER_FPGA_AUDIO_OFFLOAD
bool SPUChannel::AdvanceFPGAAudioOffload(u32 cycles)
{
    if (!(Cnt & (1<<31)))
        return false;

    const u32 type = (Cnt >> 29) & 0x3;
    // The normal dispatcher ignores format 3 on channels 0..7.
    if (type == 3 && Num < 8)
        return true;
    // Match SPUChannel::Run(): malformed short sampled channels remain
    // enabled but do not start or advance.
    if (type < 3 && (Length + LoopPos) < 16)
        return true;
    ...
    while (Timer >> 16)
    {
        ...
        if (type == 0)
        {
            ...
            if (Pos < 0)
                continue;
            if (Pos >= static_cast<s32>(LoopPos + Length))
            {
                const u32 repeat = (Cnt >> 27) & 0x3;
                if (repeat & 1)
                    Pos = static_cast<s32>(LoopPos);
                else if (repeat & 2)
                    Cnt &= ~(1<<31);
            }
        }
        else if (type == 1)
        {
            ...
            if (Pos < 0)
                continue;
            if ((Pos << 1) >= static_cast<s32>(LoopPos + Length))
            {
                const u32 repeat = (Cnt >> 27) & 0x3;
                if (repeat & 1)
                    Pos = static_cast<s32>(LoopPos >> 1);
                else if (repeat & 2)
                    Cnt &= ~(1<<31);
            }
        }
        ...
        {
            // PSG phase is not software-visible, but retaining its position
            // makes a diagnostic transition back to HPS rendering coherent.
            Pos++;
        }
...
#if NDS4MISTER_FPGA_AUDIO_OFFLOAD
bool SPUCaptureUnit::AdvanceFPGAAudioOffload(u32 cycles)
{
    if (!(Cnt & (1<<7)))
        return false;
...
void SPU::Mix(u32 spucycles)
{
    ...
    if (FPGAAudioOffload)
    {
        // The standalone MiSTer FPGA sound engine owns channel/capture time
        // when the external scheduler explicitly transfers that authority.
        // A stale callback must neither mutate the HPS shadow nor recreate
        // the 1,024-cycle event that would cap every external time window.
        if (FPGAAudioTimeAuthority)
            return;
        ++FPGAAudioOffloadCounters.MixCallbacks;
        if (Cnt & (1<<15))
        {
            for (SPUChannel& channel : Channels)
            {
                if (channel.AdvanceFPGAAudioOffload(spucycles))
                    ++FPGAAudioOffloadCounters.ChannelAdvanceCallbacks;
            }
            for (SPUCaptureUnit& capture : Capture)
            {
                if (capture.AdvanceFPGAAudioOffload(spucycles))
                    ++FPGAAudioOffloadCounters.CaptureAdvanceCallbacks;
            }
        }
        // Microphone time is independent of the speaker renderer and remains
        // part of the HPS peripheral model.
        NDS.Mic.Advance(spucycles << 1);
        NDS.ScheduleEvent(Event_SPU, true, MixInterval, 0, MixInterval >> 1);
        return;
    }
    // This counter is deliberately after the offload early return. A live
    // offload run must therefore report exactly zero HPS render callbacks.
    ++FPGAAudioOffloadCounters.HPSRenderCallbacks;
#endif
#if NDS4MISTER_NO_AUDIO_MIX
    (void)spucycles;
    NDS.ScheduleEvent(Event_SPU, true, MixInterval, 0, MixInterval >> 1);
    return;
#endif
...
#if NDS4MISTER_FPGA_AUDIO_OFFLOAD
void SPU::SetFPGAAudioOffload(bool enable)
{
    if (FPGAAudioOffload == enable)
        return;
    if (!enable && FPGAAudioTimeAuthority)
        SetFPGAAudioTimeAuthority(false);
    FPGAAudioOffload = enable;
    FPGAAudioOffloadCounters = {};
}

bool SPU::SetFPGAAudioTimeAuthority(bool enable)
{
    if (enable && !FPGAAudioOffload)
        return false;
    if (FPGAAudioTimeAuthority == enable)
        return true;

    NDS.CancelEvent(Event_SPU);
    FPGAAudioTimeAuthority = enable;
    if (!enable)
        NDS.ScheduleEvent(Event_SPU, false, 1024, 0, 0);
    return true;
}
