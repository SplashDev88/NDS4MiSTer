...
std::pair<ROMBuffer, u32> PadToPowerOf2(ROMBuffer&& data, u32 len) noexcept
{
    if (!data || len == 0)
        return {ROMBuffer(), 0};
    ...
    data = ROMBuffer();
    return {ROMBuffer::FromUnique(std::move(newdata), newlen), newlen};
}
...
{
    return ParseROM(ROMBuffer::FromUnique(std::move(romdata), romlen), romlen, userdata, std::move(args));
}

std::unique_ptr<CartCommon> ParseROM(ROMBuffer&& romdata, u32 romlen, void* userdata, std::optional<NDSCartArgs>&& args)
{
    if (!romdata)
    {
...
void NDSCartSlot::SetCart(std::unique_ptr<CartCommon>&& cart) noexcept
{
    ...
    if (header.ARM9ROMOffset >= 0x4000 && header.ARM9ROMOffset < 0x8000)
    {
        ...
        {
            u8* writableCartROM = Cart->GetWritableROM(header.ARM9ROMOffset, 0x800);
            if (!writableCartROM)
            {
                Log(LogLevel::Error, "Failed to make cart secure area writable\n");
                return;
            }
            Log(LogLevel::Debug, "Re-encrypting cart secure area\n");

            strncpy((char*)&writableCartROM[header.ARM9ROMOffset], "encryObj", 8);

            Key1_InitKeycode(false, romparams.GameCode, 3, 2);
            for (u32 i = 0; i < 0x800; i += 8)
                Key1_Encrypt((u32*)&writableCartROM[header.ARM9ROMOffset + i]);

            Key1_InitKeycode(false, romparams.GameCode, 2, 2);
            Key1_Encrypt((u32*)&writableCartROM[header.ARM9ROMOffset]);
