...
#ifndef NDS4MISTER_EXTERNAL_TIME_WINDOW
#define NDS4MISTER_EXTERNAL_TIME_WINDOW 0
#endif
...
// Default-off host/simulator seam for proving a future HPS-produced event
// horizon.  `ProcessedThrough` is the scheduler time whose due callbacks and
// resulting GPU/DMA/IRQ effects have been closed.  `RunSafeThrough` is a
// separate inclusive promise that no still-pending HPS-authoritative event can
// occur through that timestamp.
struct ExternalTimeWindow
{
    u64 ProcessedThrough = 0;
    u64 RunSafeThrough = 0;
    u32 LastEventSequence = 0;
    u32 Epoch = 0;
    u32 GrantSequence = 0;
    u32 ReplacesGrantSequence = 0;
    u64 VerifiedProducerFence = 0;
    bool ReplacesBlockingMMIO = false;
    u32 BarrierSourceSequence = 0;
    u32 BarrierSequence = 0;
    u64 BarrierTimestamp = 0;
};

// Read-only diagnostic counters for the HPS scheduler events that actually
// cap external run-safe grants. They do not participate in the protocol or
// alter scheduling. Event indices are the Event_* enum values above.
struct ExternalTimeWindowProfile
{
    u64 ClosureCount = 0;
    u64 FiniteBoundLimitedCount = 0;
    u64 NoEventCount = 0;
    std::array<u64, Event_MAX> LimitingEventCount {};
    std::array<u64, Event_MAX> GrantedCycles {};
};

// Identity assigned by the verified producer to one complete P/R/event
// publication. A replacement names the exact prior grant it supersedes.
// Sequence zero is reserved so Reset cannot be mistaken for a continuation.
...
// Immutable description of the single blocking MMIO operation stopped in the
// FPGA. Access is log2(bytes): 0/1/2 for byte/halfword/word. The active grant
// fields bind admission to the exact verified P/R/event publication, while
// SourceSequence and VerifiedProducerFence prove that this request is already
// inside the verified producer prefix.
struct ExternalBlockingMMIORequest
{
    u32 Epoch = 0;
    u32 ActiveGrantSequence = 0;
    u64 ActiveProcessedThrough = 0;
    u64 ActiveRunSafeThrough = 0;
    u32 ActiveEventSequence = 0;
    u32 SourceSequence = 0;
    u32 BarrierSequence = 0;
    u64 VerifiedProducerFence = 0;
    u64 ARM9NormalizedTimestamp = 0;
    u64 ARM7NormalizedTimestamp = 0;
    u32 CPU = 0;
    bool Write = false;
    u32 Access = 0;
    u32 Address = 0;
...
enum class ExternalTimeWindowResult
{
    Success,
    CapabilityDisabled,
    NotEnabled,
    ProtocolFaulted,
    UnsupportedAuthoritativeEventMask,
    InvalidFiniteBound,
    BoundBeforeProcessed,
    CausalTargetNotReached,
    NoActiveGrant,
    InvalidCPU,
    CPUProgressRegressed,
    CPUProgressBeyondRunSafe,
    TimestampOverflow,
    BoundarySkipped,
    SchedulerAdvancedOutsideClosure,
    FrontierRegressed,
    LateEvent,
    EventBeforeClosureTarget,
    ClosureDidNotConverge,
    DMAClosureFailed,
    UnrepresentableSideEffect,
    EventSequenceExhausted,
    InvalidExternalIFWrite,
    ExternalIFWriteReplay,
    ExternalIFWriteStateMismatch,
    ExternalIFWriteFailed,
    InvalidBlockingMMIOBarrierIdentity,
    BlockingMMIOBarrierReplay,
    BlockingMMIOBarrierStateMismatch,
    InvalidExternalTimeWindowIdentity,
    ExternalTimeWindowIdentityReplay,
    BlockingMMIOWindowIdentityMismatch,
    InvalidBlockingMMIORequest,
    BlockingMMIOAccessRequired,
    BlockingMMIOAccessAlreadyClaimed,
    BlockingMMIOAccessDescriptorMismatch,
    BlockingMMIOAccessFailed,
};
enum
{
    IRQ_VBlank = 0,
...
public: // TODO: Encapsulate the rest of these members
    ...
    u16 PowerControl9;
    FramePerformanceStats LastFramePerformance;
...
public: // TODO: Encapsulate the rest of these members
    ...
    u32 RunFrame();
    // NDS4MiSTer hybrid mode: advance peripherals using cycles executed by
    // an external FPGA CPU without running melonDS's software CPU.
    u64 AdvanceExternalCPU(u32 cpu, u32 cycles);

    // Hybrid-mode opt-in: let AdvanceExternalCPU pull a *halted* peer CPU
    // forward to the next scheduled event, so a halted core stops throttling
    // min(ARM9, ARM7) and the shared LCD/audio/DMA scheduler. Off by default;
    // the responder enables it via NDS4MISTER_HALTED_PEER_ADVANCE.
    bool ExternalHaltedPeerAdvance = false;

    // NDS4MiSTer hybrid observer. Every architectural DS SetIRQ call reports
    // its CPU and one-bit IF mask, including events raised while consuming a
    // posted write or advancing shared scheduler time. The observer is null
    // by default and never changes melonDS IRQ behavior.
    using ExternalIRQSetSink = void (*)(u32 cpu, u32 mask, void* userdata);
    void SetExternalIRQSetSink(
        ExternalIRQSetSink sink, void* userdata) noexcept
    {
        ExternalIRQSink = sink;
        ExternalIRQSinkUserdata = userdata;
    }
    ExternalIRQSetSink ExternalIRQSink = nullptr;
    void* ExternalIRQSinkUserdata = nullptr;

    // The event-window experiment is deliberately inert unless both the
    // build-time capability and this runtime opt-in are enabled.  The only
    // initially supported FPGA-authoritative scheduler-event mask is zero;
    // nonzero masks fail closed until individual event owners are proven.
    using ExternalIRQTransitionSink = bool (*)(
        const ExternalIRQTransition& transition, void* userdata) noexcept;
    void SetExternalIRQTransitionSink(
        ExternalIRQTransitionSink sink, void* userdata) noexcept
    {
        ExternalIRQTransitionSink_ = sink;
        ExternalIRQTransitionSinkUserdata_ = userdata;
    }
    bool SetExternalTimeWindowEnabled(bool enabled) noexcept;
    // Deliberately inaccurate, default-off offline beta mode.  It freezes the
    // RTC and makes WiFi inert before an external-time-window epoch begins.
    // Disable requires constructing a new NDS instance.
    bool SetExternalOfflineFastBeta(bool enabled) noexcept;
    bool ExternalOfflineFastBetaEnabled() const noexcept
    {
        return ExternalOfflineFastBeta_;
    }
    static bool SelfTestExternalOfflineFastBeta();
    // Local LCD mode leaves cadence, IRQ, DMA, and scheduler ownership in
    // FPGA. HPS may only mirror a completed descriptor and run its renderer
    // phase; no stock LCD event callback is called by this seam.
    bool SetExternalLCDRendererEnabled(bool enabled) noexcept;
    bool ApplyExternalLCDRendererPhase(
        u32 kind, u32 line, u32 vcount, u32 dispstat9, u32 dispstat7,
        u32 frameSequence, bool render, bool resync) noexcept;
    ExternalTimeWindowResult ReportExternalTimeWindowCPUReached(
        u32 cpu, u64 normalizedTimestamp) noexcept;
    ExternalTimeWindowResult AdvanceAndCloseExternalTimeWindow(
        u64 closeThrough, u64 finiteBound,
        u64 fpgaAuthoritativeEventMask,
        ExternalTimeWindow& out) noexcept;
    ExternalTimeWindowResult CloseAndQueryExternalTimeWindow(
        u64 finiteBound, u64 fpgaAuthoritativeEventMask,
        ExternalTimeWindow& out) noexcept;
    ExternalTimeWindowResult AdvanceAndCloseExternalTimeWindowVerified(
        u64 closeThrough, u64 finiteBound,
        u64 fpgaAuthoritativeEventMask,
        const ExternalTimeWindowReplacement& replacement,
        ExternalTimeWindow& out) noexcept;
    ExternalTimeWindowResult CloseAndQueryExternalTimeWindowVerified(
        u64 finiteBound, u64 fpgaAuthoritativeEventMask,
        const ExternalTimeWindowReplacement& replacement,
        ExternalTimeWindow& out) noexcept;
    // Stop both external CPUs at instruction boundaries for one exactly named
    // blocking HPS MMIO. Admission binds the immutable request to the verified
    // active epoch/group/P/R/event/fence publication. The model inserts
    // bus-wait skew up to B=max(P,T9,T7), truncates the old inclusive grant to
    // B, and keeps ordered IRQ capture active across the matching one-shot
    // access. CloseAndQueryExternalTimeWindowVerified must then atomically
    // publish a replacement that names the old group and echoes B/barrier IDs.
    ExternalTimeWindowResult BeginExternalBlockingMMIOBarrier(
        const ExternalBlockingMMIORequest& request,
        u64& barrierTimestamp) noexcept;
    bool ExternalBlockingMMIOBarrierPending() const noexcept
    {
#if NDS4MISTER_EXTERNAL_TIME_WINDOW
        return ExternalBlockingMMIOBarrierActive_;
#else
        return false;
#endif
    }
    ExternalTimeWindowResult ClaimExternalBlockingMMIOAccess(
        u32 cpu, bool write, u32 access, u32 address,
        u32 writeData, u32 executionPC) noexcept;
    ExternalTimeWindowResult FinishExternalBlockingMMIOAccess(
        bool accessSucceeded) noexcept;
    // Mirror one ARM9 IF word W1C that the external FPGA CPU has already
    // committed locally.  The operation is deliberately narrower than a
    // generic posted-MMIO seam: exact address/width, source order, grant time,
    // complete final IF, and GXFIFO parity are all checked before success.
    // Its CPU-originated IF transitions are validated and consumed locally,
    // never republished as scheduler-originated ETW records.
    ExternalTimeWindowResult ApplyExternalARM9IFW1C(
    ...
    const ExternalTimeWindowProfile& GetExternalTimeWindowProfile() const noexcept
    {
        return ExternalTimeWindowProfile_;
    }
    static bool SelfTestExternalTimeWindow();

    // Hybrid-mode opt-in: advance the whole system coherently to the next
    // scheduled event and report the timestamp reached. This is melonDS's own
    // NextTarget()/RunSystem() path, not a one-sided CPU advance, so shared
    // events actually fire. Returns SysTimestamp after the step.
    u64 AdvanceExternalSystemToNextEvent(u64 bound);

    // Hybrid mode has no melonDS CPU loop to honor StopCPU() after an
    // external MMIO write starts DMA. Complete that CPU's pending DMA before
    // the mailbox response releases the FPGA CPU. Returns false only if the
    // bounded drain cannot quiesce the DMA stop mask.
    bool CompleteExternalDMA(u32 cpu);

    // Hybrid diagnostic: true while an ARM9 channel is mid-transfer in
    // GXFIFO mode. WriteToGXFIFO is a stateful packed decoder, so a CPU
    // write landing during a DMA burst desyncs it.
    bool ExternalGXFIFODMAActive() const;
    ...
    u8 GetWRAMCnt() const noexcept { return WRAMCnt; }
    // NDS4MiSTer standalone mode: put both WRAM blocks in HPS/FPGA-shared
    // physical DDR, then rebuild the CPU bank views without changing WRAMCNT.
    void ReplaceWRAMBacking(u8* shared, u8* arm7);
...
protected:
    ...
    u32 SchedListMask;
    u64 SysTimestamp;
    bool ExternalSchedulerStarted = false;
    u8 WRAMCnt;
    u8 PostFlag9;
    u8 PostFlag7;
    u16 PowerControl7;
    ...
    ExternalIRQTransitionSink ExternalIRQTransitionSink_ = nullptr;
    void* ExternalIRQTransitionSinkUserdata_ = nullptr;
    bool ExternalTimeWindowEnabled_ = false;
    bool ExternalOfflineFastBeta_ = false;
    bool ExternalLCDRendererEnabled_ = false;
    bool ExternalTimeWindowClosureActive_ = false;
    bool ExternalTimeWindowFaulted_ = false;
    bool ExternalTimeWindowObserverFailed_ = false;
    bool ExternalTimeWindowHaveFrontier_ = false;
    u32 ExternalTimeWindowEventSequence_ = 0;
    u64 ExternalTimeWindowLastProcessed_ = 0;
    u64 ExternalTimeWindowLastRunSafe_ = 0;
    ExternalTimeWindowProfile ExternalTimeWindowProfile_ {};
    u64 ExternalTimeWindowCPUReached_[2] {};
    u32 ExternalTimeWindowObservedIF_[2] {};
    bool ExternalTimeWindowHaveVerifiedGrant_ = false;
    bool ExternalTimeWindowVerifiedCloseActive_ = false;
    u32 ExternalTimeWindowVerifiedEpoch_ = 0;
    u32 ExternalTimeWindowVerifiedGrantSequence_ = 0;
    u64 ExternalTimeWindowVerifiedProducerFence_ = 0;
    // One epoch-scoped source stream feeds every externally mirrored CPU
    // operation.  Per-seam replay guards are not sufficient: reusing one
    // source ID once as IF-W1C and once as blocking MMIO would attach two
    // architectural mutations to the same producer record.
    bool ExternalTimeWindowHaveSourceSequence_ = false;
    u32 ExternalTimeWindowLastSourceSequence_ = 0;
    bool ExternalARM9IFW1CActive_ = false;
    bool ExternalARM9IFW1CFailed_ = false;
    bool ExternalARM9IFW1CHaveLast_ = false;
    bool ExternalARM9IFW1CExpectedGXFIFOSet_ = false;
    u8 ExternalARM9IFW1CPhase_ = 0;
    u8 ExternalARM9IFW1CExpectedPhases_ = 0;
    u32 ExternalARM9IFW1CExpectedClearMask_ = 0;
    u32 ExternalARM9IFW1CLastSourceSequence_ = 0;
    u64 ExternalARM9IFW1CLastTimestamp_ = 0;
    bool ExternalBlockingMMIOBarrierActive_ = false;
    bool ExternalBlockingMMIOAccessClaimed_ = false;
    bool ExternalBlockingMMIOAccessComplete_ = false;
    bool ExternalBlockingMMIOHaveIdentity_ = false;
    u32 ExternalBlockingMMIOEpoch_ = 0;
    u32 ExternalBlockingMMIOLastSourceSequence_ = 0;
    u32 ExternalBlockingMMIOLastBarrierSequence_ = 0;
    u64 ExternalBlockingMMIOBarrierTimestamp_ = 0;
    ExternalBlockingMMIORequest ExternalBlockingMMIORequest_ {};
    u32 ExternalBlockingMMIOIEAfterAccess_[2] {};
    u32 ExternalBlockingMMIOIMEAfterAccess_[2] {};
    u32 ExternalBlockingMMIOIF2AfterAccess_ = 0;
    u32 ExternalBlockingMMIOIE2AfterAccess_ = 0;
    u32 ExternalBlockingMMIONonDMAStopAfterAccess_ = 0;
    void RecordExternalIRQTransition(u32 cpu, bool set, u32 mask) noexcept;
    void ClearIRQMask(u32 cpu, u32 mask);
    u64 NextTarget();
    u64 NextTargetSleep();
