...
// Diagnostic only, no behaviour change. ClipSegment is a free template
// function with no GPU3D access, so these live at namespace scope.
extern u64 NDS4MiSTerClipZeroDen;
extern u64 NDS4MiSTerClipInterp;
extern u64 NDS4MiSTerGXInvalidCmd;
extern u64 NDS4MiSTerGXExecuted;
extern u64 NDS4MiSTerGXInvalidHist[256];
extern u32 NDS4MiSTerGXTrace[64][5];
extern u32 NDS4MiSTerGXTraceIdx;
extern bool NDS4MiSTerGXTraceDone;
class GPU;
...
public:
    ...
    // An external replay owner supplies the authoritative GX command stream;
    ...
    void SetHighResolutionCoordinatesEnabled(bool enabled) noexcept
    {
        HighResolutionCoordinatesEnabled = enabled;
    }
...
public:
    ...
    // Hybrid mode: 64 entries is a hardware-accurate depth that assumes the
    // ARM9 stalls on a full GX FIFO. Our ARM9 runs on the FPGA and has no stall
    // signal, so it keeps writing and FIFO::Write silently discards past the
    // end -- measured at 3455 dropped commands in one NSMB intro, which is the
    // stray-polygon corruption. Size the queue so the overrun is buffered
    // instead of lost. CmdFIFOEntry is 8 bytes, so 16384 entries is 128 KiB.
    FIFO<CmdFIFOEntry, 16384> CmdStallQueue {};
    // Hybrid diagnostic: geometry commands discarded because the stall queue
    // was full. Real hardware stalls the ARM9 on a full GX FIFO; the FPGA CPU
    // has no such signal, so an overrun silently loses commands here.
    u64 GXCommandDrops = 0;
