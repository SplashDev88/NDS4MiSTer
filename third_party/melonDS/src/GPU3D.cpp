...
u64 NDS4MiSTerClipZeroDen = 0;
// Command-stream desync detector. A GX command byte outside the valid set means
// the decoder is interpreting parameter data as a command, i.e. the stream has
// lost sync -- which would produce garbage geometry.
u64 NDS4MiSTerGXInvalidCmd = 0;
u64 NDS4MiSTerGXExecuted = 0;
u64 NDS4MiSTerGXInvalidHist[256] = {0};
// Ring of recent WriteToGXFIFO inputs plus decoder state, dumped once at the
// first invalid opcode so the desync's origin is visible rather than inferred.
u32 NDS4MiSTerGXTrace[64][5] = {{0}};
u32 NDS4MiSTerGXTraceIdx = 0;
bool NDS4MiSTerGXTraceDone = false;
// Raw GXFIFO input stream. melonDS's ARM9 bus trace covers memory writes but
// not I/O writes, so the geometry command stream is invisible there. Dump it
// at the source instead, in the same order the CPU issues it, so a hardware
// capture of the mailbox GX writes can be diffed against it directly.
std::FILE* NDS4MiSTerGXStream = nullptr;
u64 NDS4MiSTerGXStreamSeq = 0;
u64 NDS4MiSTerGXStreamLimit = 0;
u64 NDS4MiSTerClipInterp = 0;
using Platform::Log;
...
{
    const auto factors = NDS4MiSTerResolveGXClipFactors<plane>(
        vin->Position[comp],
        vin->Position[3],
        vout->Position[comp],
        vout->Position[3]);
    ...
    if (factors.Widened)
    {
        if (factors.Denominator != -4294967296LL &&
            factors.Denominator != 4294967296LL)
        {
            Log(
                LogLevel::Error,
                "invalid widened GX clipping edge comp=%d plane=%d den=%lld\n",
                comp,
                plane,
                static_cast<long long>(factors.Denominator));
            abort();
        }

#define INTERPOLATE_WIDE(var) \
        { \
            outbuf->var = NDS4MiSTerGXClipInterpolateWide( \
                vin->var, vout->var, factors); \
        }

        if (comp != 0) INTERPOLATE_WIDE(Position[0]);
        if (comp != 1) INTERPOLATE_WIDE(Position[1]);
        if (comp != 2) INTERPOLATE_WIDE(Position[2]);
        INTERPOLATE_WIDE(Position[3]);
        outbuf->Position[comp] = static_cast<s32>(
            static_cast<s64>(plane) * outbuf->Position[3]);

        if (attribs)
        {
            INTERPOLATE_WIDE(Color[0]);
            INTERPOLATE_WIDE(Color[1]);
            INTERPOLATE_WIDE(Color[2]);

            INTERPOLATE_WIDE(TexCoords[0]);
            INTERPOLATE_WIDE(TexCoords[1]);
        }

        outbuf->Clipped = true;
        return;

#undef INTERPOLATE_WIDE
    }

    const s64 factor_num = factors.Numerator;
    const s32 factor_den = static_cast<s32>(factors.Denominator);
    const NDS4MiSTerGXClipDivider divider(factor_den);
    NDS4MiSTerClipInterp++;
    if (factor_den == 0) NDS4MiSTerClipZeroDen++;

#define INTERPOLATE(var) \
    { \
        const s64 delta = static_cast<s64>(vout->var) - vin->var; \
        outbuf->var = vin->var + divider.divide(delta * factor_num); \
    }
...
void GPU3D::SubmitPolygon() noexcept
{
    ...
    for (int i = clipstart; i < nverts; i++)
    {
        ...
        // High-resolution coordinates are consumed only by the OpenGL and
        // compute renderers. Native software rendering uses FinalPosition;
        // avoid two 64-bit divisions per vertex when its owner has selected
        // that fixed path. Keep a deterministic native-resolution fallback
        // rather than retaining stale vertex data.
        if (HighResolutionCoordinatesEnabled && w != 0)
        {
        ...
        else if (!HighResolutionCoordinatesEnabled)
        {
            vtx->HiresPosition[0] = vtx->FinalPosition[0] << 4;
            vtx->HiresPosition[1] = vtx->FinalPosition[1] << 4;
        }
    }
    ...
    for (int i = 0; i < nverts; i++)
    {
        ...
        if (FlushAttributes & 0x2)
            ...
            z = (NDS4MiSTerGXDivideZ(
                    vtx->Position[2],
                    static_cast<u32>(vtx->Position[3])) + 0x3FFF) * 0x200;
        else
            z = 0x7FFE00;
...
void GPU3D::CmdFIFOWrite(const CmdFIFOEntry& entry) noexcept
{
    ...
    else
    {
        if (CmdFIFO.IsFull())
        {
            ...
            if (CmdStallQueue.IsFull()) GXCommandDrops++;
...
GPU3D::CmdFIFOEntry GPU3D::CmdFIFORead() noexcept
{
    ...
    if (CmdPIPE.Level() <= 2)
    {
        ...
        // Hybrid rendering replays an already-authoritative GX stream. Its
        // ARM9 and DMA controller remain on the FPGA, so walking four idle
        // melonDS DMA channels and rewriting an unobserved IRQ on every pipe
        // refill is pure duplicate work. Keep the normal emulator behavior
        // unchanged for every non-external caller.
...
void GPU3D::ExecuteCommand() noexcept
{
    ...
    const bool valid =
        ...
        c == 0x50 || c == 0x60 ||
        (c >= 0x70 && c <= 0x72);
    if (!valid) { NDS4MiSTerGXInvalidCmd++; NDS4MiSTerGXInvalidHist[c]++; }
...
void GPU3D::VBlank() noexcept
{
    if (GeometryEnabled)
    {
        ...
        if (NDS4MiSTer::Trace2DEnabled())
            NDS4MiSTer::EmitTrace3DFrame(NDS.NumFrames, NDS.ARM9Timestamp,
                NumVertices, NumPolygons, FlushAttributes);
...
{
    if (!NDS4MiSTerGXStreamSeq && !NDS4MiSTerGXStream) {
        if (const char* path = std::getenv("NDS4MISTER_GX_STREAM")) {
            NDS4MiSTerGXStream = std::fopen(path, "w");
            const char* lim = std::getenv("NDS4MISTER_GX_STREAM_LIMIT");
            NDS4MiSTerGXStreamLimit = lim ? std::strtoull(lim, nullptr, 0)
                                          : 200000;
        }
    }
    if (NDS4MiSTerGXStream &&
        NDS4MiSTerGXStreamSeq < NDS4MiSTerGXStreamLimit) {
        std::fprintf(NDS4MiSTerGXStream, "%08x\n", val);
        if ((NDS4MiSTerGXStreamSeq & 0x3ff) == 0)
            std::fflush(NDS4MiSTerGXStream);
    }
    NDS4MiSTerGXStreamSeq++;

    if (!NDS4MiSTerGXTraceDone) {
        auto* r = NDS4MiSTerGXTrace[NDS4MiSTerGXTraceIdx & 63];
        r[0] = val; r[1] = NumCommands; r[2] = CurCommand;
        r[3] = ParamCount; r[4] = TotalParams;
        NDS4MiSTerGXTraceIdx++;
    }
    if (NumCommands == 0)
    {
        NumCommands = 4;
    ...
    for (;;)
    {
        if ((CurCommand & 0xFF) || (NumCommands == 4 && CurCommand == 0))
        {
            ...
            entry.Param = val;
            // Catch the desync where it is CREATED. Checking at execute time is
            // useless: the 256-deep FIFO decouples enqueue from execute, so the
            // write ring has already scrolled past the culprit by then.
            if (!NDS4MiSTerGXTraceDone) {
                const u8 ec = entry.Command;
                const bool eok =
                    ec == 0x00 || (ec >= 0x10 && ec <= 0x1C) ||
                    (ec >= 0x20 && ec <= 0x2B) || (ec >= 0x30 && ec <= 0x34) ||
                    (ec >= 0x40 && ec <= 0x41) || ec == 0x50 || ec == 0x60 ||
                    (ec >= 0x70 && ec <= 0x72);
                if (!eok) {
                    NDS4MiSTerGXTraceDone = true;
                    Platform::Log(Platform::LogLevel::Error,
                        "NDS4MISTER_GXDESYNC enqueue cmd=%02x param=%08x cur=%08x numcmd=%u pc=%u tot=%u\n",
                        ec, val, CurCommand, NumCommands, ParamCount, TotalParams);
                    const u32 n = NDS4MiSTerGXTraceIdx < 64 ? NDS4MiSTerGXTraceIdx : 64;
                    const u32 start = NDS4MiSTerGXTraceIdx >= 64 ? NDS4MiSTerGXTraceIdx - 64 : 0;
                    for (u32 k = 0; k < n; ++k) {
                        const auto* r = NDS4MiSTerGXTrace[(start + k) & 63];
                        Platform::Log(Platform::LogLevel::Error,
                            "GXW[%u] val=%08x numcmd=%u cur=%08x pc=%u tot=%u\n",
                            k, r[0], r[1], r[2], r[3], r[4]);
                    }
                }
            }
            if (NDS4MiSTer::Trace2DEnabled())
                NDS4MiSTer::EmitTrace3DCommand(NDS.NumFrames, NDS.ARM9Timestamp,
                    entry.Command, entry.Param);
            CmdFIFOWrite(entry);
        }
...
void GPU3D::WriteExternalNormalizedCommand(u8 command, u32 val) noexcept
{
...
void GPU3D::Write8(u32 addr, u8 val) noexcept
{
    ...
    if (NDS4MiSTer::Trace2DEnabled())
        NDS4MiSTer::EmitTrace3DRegister(NDS.NumFrames, NDS.ARM9Timestamp, addr, val, 1);
...
void GPU3D::Write16(u32 addr, u16 val) noexcept
{
    ...
    if (!GeometryEnabled  && addr >= 0x04000400 && addr < 0x04000700) return;
    if (NDS4MiSTer::Trace2DEnabled())
        NDS4MiSTer::EmitTrace3DRegister(NDS.NumFrames, NDS.ARM9Timestamp, addr, val, 2);
...
void GPU3D::Write32(u32 addr, u32 val) noexcept
{
    if (!RenderingEnabled && addr >= 0x04000320 && addr < 0x04000400) return;
    if (!GeometryEnabled  && addr >= 0x04000400 && addr < 0x04000700) return;
    if (NDS4MiSTer::Trace2DEnabled() && !(addr >= 0x04000400 && addr < 0x040005CC))
        NDS4MiSTer::EmitTrace3DRegister(NDS.NumFrames, NDS.ARM9Timestamp, addr, val, 4);
