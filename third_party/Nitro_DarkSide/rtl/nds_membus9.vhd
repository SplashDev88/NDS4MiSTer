...
entity nds_membus9 is
   ...
   (
      is_simu : std_logic := '0';
      -- NDS4MiSTer keeps palette, OAM and all otherwise-unmapped authority
      -- outside this donor island. The upstream default stays unchanged.
      cold_external_enable : boolean := false
   );
   port
   (
      clk            : in  std_logic;
      ...
      wsh_low        : out std_logic_vector(1 downto 0) := "00";
      ...
      vram_low       : out std_logic_vector(1 downto 0) := "00";
      ...
      io_low         : out std_logic_vector(1 downto 0) := "00";
      io_wired_out   : in  std_logic_vector(31 downto 0);
      io_wired_done  : in  std_logic;

      -- Optional ordered path for the regions that upstream Nitro serves
      -- locally without a response handshake. NDS4MiSTer uses this path to
      -- retain its current video-memory and HPS-oracle authority.
      cold_ena       : out std_logic := '0';
      cold_rnw       : out std_logic := '1';
      cold_addr      : out std_logic_vector(31 downto 0) := (others => '0');
      cold_acc       : out std_logic_vector(1 downto 0) := "10";
      cold_din       : out std_logic_vector(31 downto 0) := (others => '0');
      cold_dout      : in  std_logic_vector(31 downto 0) := (others => '0');
      cold_done      : in  std_logic := '0';
...
architecture arch of nds_membus9 is
   ...
   type t_target is (T_ITCM, T_DTCM, T_DTCM_OUTER, T_BROM, T_MAIN,
                     T_WRAMSH, T_IO, T_VRAM, T_PAL, T_OAM, T_OPEN);
   ...
   signal dtcm_outer_state_hit : std_logic;
   signal dtcm_region_mask : unsigned(32 downto 0);
   signal dtcm_effective_base : unsigned(31 downto 0);
...
begin
   ...
   (
      is_simu => is_simu,
      dcache_enable => not cold_external_enable,
      serialized_fill => cold_external_enable
   )
   port map
   (
      clk           => clk,
   ...
   accept_now <= '1' when reset = '0' and
                          (state = IDLE or state = FINISH or
                           ...
                           (state = W_IO_RESP and io_wired_done = '1') or
                           (state = W_COLD and cold_done = '1')) else '0';
   ...
   dtcm_sel       <= accept_now and cpu_ena and dtcm_hit and not itcm_hit and
                     not dtcm_outer_state_hit;
   ...
   -- r343 already owns the SDK handler state at +0x3C0 and the final DTCM word
   -- that protects IRQ delivery at +0x3FFC. Keep only those full addresses on
   -- the cold boundary. The effective base uses the same architectural
   -- alignment mask as dtcm_hit.
   dtcm_region_mask <= region_mask(dtcm_size);
   dtcm_effective_base <= unsigned(dtcm_base & x"000") and
      dtcm_region_mask(31 downto 0);
   dtcm_outer_state_hit <= '1' when cold_external_enable and
      dtcm_hit = '1' and
      (unsigned(cpu_adr(31 downto 2)) =
          dtcm_effective_base(31 downto 2) + to_unsigned(16#0F0#, 30) or
       unsigned(cpu_adr(31 downto 2)) =
          dtcm_effective_base(31 downto 2) + to_unsigned(16#FFF#, 30))
      else '0';
   ...
   begin
      ...
      if (itcm_hit = '1') then
         dec_target <= T_ITCM;
      elsif (dtcm_outer_state_hit = '1') then
         dec_target <= T_DTCM_OUTER;
      elsif (dtcm_hit = '1' and dtcm_outer_state_hit = '0') then
         dec_target <= T_DTCM;
               ...
               if (cpu_ena = '1') then
                  ...
                  case dec_target is
                     ...
                     when T_DTCM_OUTER =>
                        ...
                        cold_rnw <= cpu_rnw;
                        if (cpu_rnw = '1') then
                           -- The r343 shadow returns a full word. Nitro applies
                           -- the original access width and lane on the reply.
                           cold_addr <= cpu_adr(31 downto 2) & "00";
                           cold_acc  <= ACCESS_32BIT;
                        else
                           cold_addr <= cpu_adr;
                           cold_acc  <= cpu_acc;
                        end if;
                     ...
                     when T_VRAM =>
                        ...
                        vram_be   <= be;
                        vram_din  <= wdata;
                        vram_low  <= cpu_adr(1 downto 0);
                        state     <= W_VRAM;

                     when T_PAL =>
                        if cold_external_enable then
                           cold_ena  <= '1';
                           cold_rnw  <= cpu_rnw;
                           cold_addr <= cpu_adr;
                           cold_acc  <= cpu_acc;
   ...
   cpu_done <= '1'            when state = FINISH    else
               ...
               io_wired_done  when state = W_IO_RESP else
               cold_done      when state = W_COLD    else '0';

   -- ================= read data mux + rotation (gba_mem_readrotate) =================
   din_unrot <= cold_dout when cold_external_enable and
                   (target = T_DTCM_OUTER or target = T_PAL or
                    target = T_OAM or target = T_OPEN) else
                itcm_readdata when target = T_ITCM   else
   ...
   begin
      cpu_din <= (others => '0');
      -- The NDS4MiSTer cold boundary already applies the access width and
      -- unaligned CPU-read rules. Do not apply the Nitro lane rotation a
      -- second time to a response that came from that boundary. Main RAM is
      -- different: the I-cache asks for aligned words and keeps this rotate.
      if cold_external_enable and
         (target = T_WRAMSH or target = T_VRAM or target = T_IO or
          target = T_PAL or target = T_OAM or target = T_OPEN) then
         cpu_din <= din_unrot;
      elsif (r_acc = ACCESS_8BIT) then
         case (r_low) is
