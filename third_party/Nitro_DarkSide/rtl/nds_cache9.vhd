...
entity nds_cache9 is
   ...
   (
      is_simu : std_logic := '0';
      -- NDS4MiSTer Stage B keeps the instruction cache but does not take
      -- ownership of data-cache state. The upstream default stays unchanged.
      dcache_enable : boolean := true;
      -- Keep W_MAIN owned until a complete fill is present. The upstream
      -- default keeps its critical-word-first background-fill behavior.
      serialized_fill : boolean := false
   );
   port
   (
      clk         : in  std_logic;
...
architecture arch of nds_cache9 is
   ...
   signal critical_hold : std_logic_vector(31 downto 0) := (others => '0');
   signal accepted_op_ena : std_logic;

begin

   -- D-cache maintenance is an ordered no-op when that cache is absent.
   -- I-cache operations still use the upstream state machine.
   accepted_op_ena <= op_ena when dcache_enable or
      op = "0000" or op = "0001" else '0';
   op_busy <= accepted_op_ena or op_pending or op_active;
   ...
   spec_sel <= '1' when (state = IDLE and accepted_op_ena = '0' and
                         op_pending = '0' and
                         req_ena = '0' and req_pending = '0') else '0';

   it_raddr <= to_integer(unsigned(op_addr(10 downto 5))) when (state = IDLE and accepted_op_ena = '1') else
   ...
   dt_raddr <= to_integer(unsigned(op_addr(9 downto 5))) when (state = IDLE and accepted_op_ena = '1') else
   ...
   begin
      ...
      gdtag : if dcache_enable generate
      ...
      gddata : if dcache_enable generate
      ...
      gno_dcache : if not dcache_enable generate
      begin
         dt_q(w) <= (others => '0');
         dd_q(w) <= (others => '0');
      end generate;
   ...
   begin
      if rising_edge(clk) then
         ...
         if (reset = '1') then
            ...
            if (req_ena = '1' and (state /= IDLE or
                accepted_op_ena = '1' or op_pending = '1')) then
               req_pending <= '1';
            end if;
               ...
               when FILL_WAIT =>
                  if (mem_done = '1') then
                     ...
                     if (beat = to_integer(unsigned(r_addr(4 downto 2)))) then
                        if serialized_fill then
                           critical_hold <= mem_rdata;
                        else
                           resp_rdata <= mem_rdata;
                           resp_done  <= '1';
                        end if;
