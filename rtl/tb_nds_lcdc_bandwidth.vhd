...
use std.env.all;
entity tb_nds_lcdc_bandwidth is
   generic (reply_latency : positive := 6; issue_interval : positive := 3);
end;
architecture sim of tb_nds_lcdc_bandwidth is
   ...
   signal read_count, inflight : natural := 0;
...
begin
   ...
   rsready<='1' when cycles mod issue_interval=0 and inflight<2 else '0';
   process(clk)
   ...
   begin
      if rising_edge(clk) then
         ...
         if rsreq='1' and rsready='1' then
            ...
            deadline(tail):=cycles+reply_latency;
   ...
   process
      ...
      variable y, cycle_base, read_base : natural;
   begin
      ...
      for b in 0 to 3 loop
         for line_case in 0 to 1 loop
            ...
            wait until falling_edge(clk);
            cycle_base:=cycles;read_base:=read_count;
            ...
            report "LCDC_BANDWIDTH latency=" & integer'image(reply_latency) & " interval=" & integer'image(issue_interval) & " bank=" & integer'image(b) & " y=" & integer'image(y) & " cycles=" & integer'image(cycles-cycle_base) & " sdram_reads=" & integer'image(read_count-read_base) severity note;
         end loop;
      end loop;
