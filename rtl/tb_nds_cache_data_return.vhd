...
-- Data- and instruction-return regression: consecutive ARM/Thumb fetches across all
...
begin
   ...
   process
      ...
      -- Present the following read during the current response. Cover four
      -- ways, byte/halfword lanes and word rotation using the real membus.
      procedure data_stream(mode : natural) is
         variable a, value : std_logic_vector(31 downto 0);
         variable elapsed, pair_start : natural := 0;
         function address_for(i : natural) return std_logic_vector is
            variable offset : natural;
         begin
            case mode is
               when 0 => offset := i mod 32;
               when 1 => offset := (i mod 16)*2;
               when 2 => offset := (i mod 8)*4;
               when others => offset := (i mod 8)*4 + ((i/4) mod 4);
            end case;
            return std_logic_vector(to_unsigned(16#02002000#+(i mod 4)*16#400#+offset,32));
         end;
      begin
         drained; pair_start:=pair_count;
         wait until falling_edge(clk);
         adr<=address_for(0); rnw<='1'; cached<='1'; code<='0';
         cpu_lock<='0'; dma<='0'; ena<='1';
         if mode=0 then acc<=ACCESS_8BIT;
         elsif mode=1 then acc<=ACCESS_16BIT;else acc<=ACCESS_32BIT;end if;
         cycle; ena<='0';
         for i in 0 to 127 loop
            a:=address_for(i);value:=expected(a);
            value:=std_logic_vector(rotate_right(unsigned(value),to_integer(unsigned(a(1 downto 0)))*8));
            if mode=0 then value:=x"000000"&value(7 downto 0);
            elsif mode=1 then value:=x"0000"&value(15 downto 0);end if;
            for n in 0 to 10000 loop
               exit when done='1';cycle;elapsed:=elapsed+1;
            end loop;
            assert done='1' report "data stream timeout" severity failure;
            assert din=value report "data stream mismatch mode="&integer'image(mode)&" index="&integer'image(i)&
               " expected="&to_hstring(value)&" actual="&to_hstring(din) severity failure;
            if i<127 then adr<=address_for(i+1);ena<='1';end if;
            cycle;ena<='0';elapsed:=elapsed+1;
         end loop;
         assert pair_count=pair_start report "warm data stream refilled" severity failure;
         if expect_single_cycle then
            assert elapsed=128 report "data stream not one cycle per read" severity failure;
         else
            assert elapsed=256 report "baseline data stream latency changed" severity failure;
         end if;
         for n in 0 to 3 loop
            assert done='0' report "duplicate data response" severity failure;cycle;
         end loop;
         report "PASS data stream mode="&integer'image(mode)&" cycles="&integer'image(elapsed);
         acc<=ACCESS_32BIT;
      end;
      procedure write_then_read(a, write_value, read_value : std_logic_vector(31 downto 0);
                                size : std_logic_vector(1 downto 0)) is
      begin
         wait until falling_edge(clk);
         adr<=a;dout<=write_value;rnw<='0';cached<='1';code<='0';
         cpu_lock<='0';dma<='0';acc<=size;ena<='1';cycle;ena<='0';
         for n in 0 to 10000 loop exit when done='1';cycle;end loop;
         assert done='1' report "adjacent data write timeout" severity failure;
         adr<=a(31 downto 2)&"00";rnw<='1';acc<=ACCESS_32BIT;ena<='1';
         cycle;ena<='0';
         for n in 0 to 10000 loop exit when done='1';cycle;end loop;
         assert done='1' and din=read_value report "adjacent write/read returned stale or wrong byte lanes" severity failure;
         cycle;
         for n in 0 to 2 loop assert done='0' report "duplicate adjacent data response" severity failure;cycle;end loop;
      end;
   ...
   begin
      ...
      -- New D-return coverage. D-cache ways share set0 with a 1KB stride.
      for way in 0 to 3 loop
         addr_value:=std_logic_vector(to_unsigned(16#02002000#+way*16#400#,32));
         transfer(addr_value,expected(addr_value),'1','1','0','0');drained;
      end loop;
      for mode in 0 to 3 loop data_stream(mode);end loop;
      write_then_read(x"02002000",x"11223344",x"11223344",ACCESS_32BIT);
      write_then_read(x"02002000",x"000000AA",x"112233AA",ACCESS_8BIT);
      write_then_read(x"02002001",x"000000BB",x"1122BBAA",ACCESS_8BIT);
      write_then_read(x"02002002",x"000000CC",x"11CCBBAA",ACCESS_8BIT);
      write_then_read(x"02002003",x"000000DD",x"DDCCBBAA",ACCESS_8BIT);
      write_then_read(x"02002000",x"00005566",x"DDCC5566",ACCESS_16BIT);
      write_then_read(x"02002002",x"00007788",x"77885566",ACCESS_16BIT);
      -- A locked CPU read and a DMA read must use their original qualification.
      transfer(x"02002000",x"77885566",'1','1','0','1');
      transfer(x"02002004",expected(x"02002004"),'1','1','0','1','1');
      drained;
      -- Cache maintenance wins over the prospective one-cycle data response.
      -- Drop the dirty cached word and return the deliberately changed backing.
      transfer(x"02402000",x"C0FFEE55",'0','0','0','0');drained;
      wait until falling_edge(clk);
      adr<=x"02002000";rnw<='1';cached<='1';code<='0';ena<='1';cpu_lock<='0';dma<='0';
      cycle;ena<='0';op<=x"2";op_addr<=x"00000000";op_ena<='1';
      wait until falling_edge(clk);
      assert done='0' report "D invalidation did not suppress speculative response" severity failure;
      cycle;op_ena<='0';
      for n in 0 to 10000 loop exit when done='1';cycle;end loop;
      assert done='1' and din=x"C0FFEE55" report "D invalidation returned stale data" severity failure;
      cycle;drained;
      -- A reset coinciding with a prospective data hit must cancel it.
      wait until falling_edge(clk);
      adr<=x"02002000";rnw<='1';cached<='1';code<='0';ena<='1';
      cycle;ena<='0';reset<='1';wait until falling_edge(clk);
      assert done='0' report "reset failed to suppress data response" severity failure;
      cycle;cycle;reset<='0';cycle;
      transfer(x"02002000",x"C0FFEE55",'1','1','0','0');drained;
      report "PASS D-cache streams, write-read byte lanes, lock/DMA, simultaneous invalidation and reset";
      std.env.stop; wait;
   end process;
end architecture;
