// Candidate held-until-accepted MiSTer DDR master for r34 validation.
// Kept separate from the production source while r33 compiles. Tests may
// alias it to the production module name to exercise all existing consumers.
`ifdef CPU_DDRAM_HELD_PRODUCTION_NAME
module nds_cpu_ddram #(
`else
module nds_cpu_ddram_held #(
`endif
    ...
    always_comb begin
        ...
        case (latched_access)
            ACCESS_8: begin
                ...
                ddram_write_data =
                    {56'h0, latched_write_data[7:0]} <<
                    (latched_offset * 8);
            end
            ACCESS_16: begin
                ddram_byte_enable = 8'h03 << latched_offset;
                ddram_write_data =
                    {48'h0, latched_write_data[15:0]} <<
                    (latched_offset * 8);
            end
            default: begin
                ddram_byte_enable = 8'h0f << latched_offset;
                ddram_write_data =
                    {32'h0, latched_write_data} <<
                    (latched_offset * 8);
            end
        endcase
    end
                ...
                WAIT_READ: if (ddram_read_data_ready) begin
                    case (latched_access)
                        ACCESS_8:
                            read_data <=
                                (ddram_read_data >>
                                 (latched_offset * 8)) & 32'h000000ff;
                        ACCESS_16:
                            read_data <=
                                (ddram_read_data >>
                                 (latched_offset * 8)) & 32'h0000ffff;
                        default:
                            read_data <=
                                ddram_read_data >> (latched_offset * 8);
                    endcase
                    done <= 1'b1;
                    state <= WAIT_RELEASE;
                end
