...
-- Directed r182 regression. The first SDK/IRQ/context invocation is healthy
-- and must not freeze telemetry. Only the second privileged LDM^ context
-- poisons System R11; R13 remains correct. The resulting final
-- LDMIA sp!,{r11,pc} reads 0x10/0x14 and must freeze the complete seven-stage
-- diagnosis as FFFCBF66.
entity tb_nds_arm9_sp_path_telemetry is
end entity;

architecture sim of tb_nds_arm9_sp_path_telemetry is
   ...
   signal first_r11_source, first_r15_source, second_r11_source,
      ...
      (others => '0');
   signal first_marker_bad : std_logic := '0';
   ...
   begin
      case a is
         ...
         when x"00001030" =>
            if poison_r11 = '1' then
               return x"00000000";
            else
               return x"027E3914";
            end if;
         when x"00001034" => return x"1111000C";
         when x"00001038" => return x"027E3914";
   ...
   begin
      case a is
         ...
         when x"01FFD22C" => return x"E92D4800";
         when x"01FFD230" => return x"E24DD010";
         when x"01FFD234" => return x"E1A0B00D";
         when x"01FFD238" => return x"E1A00000";
         ...
         when x"01FFD27C" => return x"E28BD010";
         when x"01FFD280" => return x"E8BD8800";
...
begin
            ...
            case responder_state is
               ...
               when RESP_DONE =>
                  ...
                  if request_rnw = '1' and
                     ...
                     (request_addr = x"027E3924" or
                      request_addr = x"027E3928" or
   ...
   process
      variable frozen_word : std_logic_vector(31 downto 0);
   begin
      reset <= '1';
      ...
      assert body_count >= 1
         report "first healthy invocation did not reach the function body"
         severity failure;
      irq <= '1';
      wait until vector_count >= 1 for 2 ms;
      assert vector_count >= 1
         report "first healthy invocation did not take IRQ"
         severity failure;
      irq <= '0';
      wait until final_return_count >= 1 for 2 ms;
      assert final_return_count >= 1
         report "first healthy invocation did not complete final POP"
         severity failure;
      wait until marker_count >= 1 for 2 ms;
      assert marker_count >= 1 and first_marker_bad = '0'
         report "first invocation did not return with healthy System SP"
         severity failure;
      assert first_r11_source = x"027E3924" and
             first_r15_source = x"027E3928"
         report "first invocation was not healthy"
         severity failure;
      wait until rising_edge(clk);
      wait for 0 ns;
      assert debug_mixed = execute_pc
         report "healthy first invocation incorrectly froze SP telemetry: " &
      ...
      assert body_count >= 2
         report "second poisoned invocation did not reach the function body"
         severity failure;
      irq <= '1';
      wait until vector_count >= 2 for 2 ms;
      assert vector_count >= 2
         report "second poisoned invocation did not take IRQ"
         severity failure;
      irq <= '0';
      wait until final_return_count >= 2 for 2 ms;
      assert final_return_count >= 2
         report "second poisoned invocation did not complete bad final POP"
         severity failure;
      assert second_r11_source = x"00000010" and
             second_r15_source = x"00000014"
         report "poisoned R11 did not drive the expected bad POP sources"
         severity failure;
      -- The responder observes its registered done pulse one clock before
      -- the CPU consumes it. Wait for the diagnostic publication itself,
      -- rather than assuming one particular delta-cycle ordering.
      wait until debug_mixed = x"FFFCBF66" for 1 us;
      assert debug_mixed = x"FFFCBF66"
         report "seven-stage SP telemetry mismatch: " &
                to_hstring(debug_mixed)
         severity failure;
      frozen_word := debug_mixed;
      for i in 1 to 16 loop
         wait until rising_edge(clk);
      end loop;
      assert debug_mixed = frozen_word
         report "bad SP telemetry did not remain frozen"
         severity failure;

      report "PASS: ARM9 seven-stage SP telemetry ignores healthy invocation and freezes poisoned privileged R11 as FFFCBF66"
         severity note;
      stop;
      wait;
   end process;
