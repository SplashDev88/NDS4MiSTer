    ...
    for local_only in false true; do
      for id in 0 1 2 3 4; do
        for stall in false true; do
            nvc --std=2008 -e -g CASE_ID="$id" -g STALL_SINK="$stall" -g LOCAL_ONLY="$local_only" tb_nds_h3d_vram9_retirement
...
echo "PASS: VRAM9 LCDC local/event acceptance, unchanged BG-only path, DMA reads, unposted pulses, backpressure and service-off"
